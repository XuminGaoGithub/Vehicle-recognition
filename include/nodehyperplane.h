...
class NodeHyperPlane : public Node
{
 public:
	...
	NodeHyperPlane(const HyperParameters &, int, float, const std::vector<int> &, const std::vector<float> & );
	NodeHyperPlane(const HyperParameters &, int, int, float, const std::vector<int> &, const std::vector<float> & );

    NodeHyperPlane(const HyperParameters &hp, int reset, const xmlNodePtr nodeNode);

    virtual xmlNodePtr save(FILE *fp) const;
...
private:
	...
	xmlNodePtr saveBestFeatures() const;
	xmlNodePtr saveBestWeights() const;
	xmlNodePtr saveBestTreshold() const;
