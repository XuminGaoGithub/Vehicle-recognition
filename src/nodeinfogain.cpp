...
NodeInfoGain::NodeInfoGain(const HyperParameters &hp, int depth) : Node(hp, depth), m_bestFeature( -1 ),
        m_bestThreshold( 0.0 )
{

}

NodeInfoGain::NodeInfoGain(const HyperParameters &hp, int depth, int reset) : Node(hp, depth, reset), m_bestFeature( -1 ),
        m_bestThreshold( 0.0 )
{

}

NodeInfoGain::NodeInfoGain(const HyperParameters &hp, int reset, const xmlNodePtr nodeNode) : Node(hp, 0, -1), m_bestFeature( -1 ),
        m_bestThreshold( 0.0 )
{
    ...
    else
    {
        ...
        while ( cur != 0 )
        {
            ...
            {
                std::string childNode = readStringProp(cur,"child");
                std::string type = readStringProp(cur,"type");
                if ( childNode == "left" )
                {
                    if (type == NODE_GINI)
                    {
...
xmlNodePtr NodeInfoGain::saveFeature() const
{
...
xmlNodePtr NodeInfoGain::save(FILE *fp) const
{
    xmlNodePtr node = xmlNewNode( NULL, reinterpret_cast<const xmlChar*>( "node" ) );
    xmlNewProp( node, reinterpret_cast<const xmlChar*>( "type" ),
                reinterpret_cast<const xmlChar*>( NODE_INFO_GAIN ) );
...
std::pair<float, float> NodeInfoGain::calcInfoGainAndThreshold(const std::vector<int>& labels,
...
std::pair<float, float> NodeInfoGain::calcInfoGainAndThreshold(const std::vector<int>& labels, const std::vector<double>& weights,
                                                               const std::vector<std::pair<float, int> >& responses)
{
    // Initialize the counters: left takes all at the begining
    double DInfo, LInfo, RInfo, LTotal, RTotal, bestW0 = 0.0, bestDInfo = 1e10;
...
std::pair<float, float> NodeInfoGain::calcClusterScoreAndThreshold(const matrix<float>& data, const std::vector<int>& inBagSamples,
                                                                    const std::vector<std::pair<float, int> >& responses) {
...
void NodeInfoGain::findHypotheses(const matrix<float>& data, const std::vector<int>& labels,
                                  const std::vector<int>& inBagSamples, const std::vector<int>& randFeatures)
{
    ...
    double bestDInfo = 1e10, bestThreshold = 0.0;
    int bestFeature = *it;
    std::pair<float,float> curInfoThresh;
    ...
    while ( it != end )
    {
        ...
        curInfoThresh = calcInfoGainAndThreshold(labels, responses);
        if (curInfoThresh.first < bestDInfo)
        {
            bestDInfo = curInfoThresh.first;
            bestFeature = *it;
            bestThreshold = curInfoThresh.second;
        }
...
void NodeInfoGain::findHypothesesLU(const matrix<float>& data, const std::vector<int>& labels,
                                  ...
                                  const std::vector<int>& inBagSamples, const std::vector<int>& randFeatures)
{
    ...
    while ( it != end )
    {
        ...
        curInfoThresh = calcInfoGainAndThreshold(labels, weights, responses);
        if (curInfoThresh.first < bestDInfo)
        {
            bestDInfo = curInfoThresh.first;
            bestFeature = *it;
...
void NodeInfoGain::evalNode(const matrix<float>& data, const std::vector<int>& inBagSamples,
...
NODE_TRAIN_STATUS NodeInfoGain::train(const matrix<float>& data, const std::vector<int>& labels, std::vector<int>& inBagSamples,
                                      matrix<float>& confidences, std::vector<int>& predictions)
{
    ...
    if ( doSplit )
    {
        ...
        m_leftChildNode = Ptr(new NodeInfoGain(m_hp,m_depth + 1));
        m_rightChildNode = Ptr(new NodeInfoGain(m_hp,m_depth + 1));
...
NODE_TRAIN_STATUS NodeInfoGain::trainLU(const matrix<float>& data, const std::vector<int>& labels, std::vector<int>& inBagSamples,
                                        matrix<float>& confidences, std::vector<int>& predictions)
{
...
NODE_TRAIN_STATUS NodeInfoGain::train(const matrix<float>& data, const std::vector<int>& labels, const std::vector<double>& weights,
...
void NodeInfoGain::eval(const matrix<float>& data, const std::vector<int>& sampleIndeces,
                        matrix<float>& confidences, std::vector<int>& predictions)
{//cout<<"test"<<endl;
    if (m_isLeaf)
    {
...
void NodeInfoGain::getPath(const matrix<float>& data, const std::vector<int>& sampleIndeces, std::vector<std::vector<int> >& path) {
  BOOST_FOREACH(int n, sampleIndeces) {
...
void NodeInfoGain::refine(const matrix<float>& data, const std::vector<int>& labels,
                  std::vector<int>& samples, matrix<float>& confidences, std::vector<int>& predictions)
{

}

void NodeInfoGain::refine(const matrix<float>& data, const std::vector<int>& labels, const std::vector<double>& weights,
                  std::vector<int>& samples, matrix<float>& confidences, std::vector<int>& predictions)
{
    // calc confidence, labels, etc
    if (!m_isLeaf)
