...
/*            if (nodeType == NODE_GINI)
            ...
            else*/ if (nodeType == NODE_HYPER_PLANE)
            {
                m_rootNode = Node::Ptr( new NodeHyperPlane( hp, 0, cur ) );
            }
        }
...
xmlNodePtr Tree::save(FILE *fp,int i) const
{
	fprintf(fp,"\t<tree num=\"%d\">\n",i);
...
{cout<<"step4"<<endl;
//cout<<"datasize1"<<data.size1()<<endl;
    // Initialize
	m_confidences.clear();
	m_predictions.clear();
	m_confidences.resize(data.size1(), m_hp.numClasses);
    m_predictions.resize(data.size1());
//cout<<"m_confidences1"<<m_confidences(12,2)<<endl;
    std::vector<int> sampleIndeces;
    sampleIndeces.reserve(data.size1());
    for (unsigned int i = 0; i < data.size1(); i++)
    {
        sampleIndeces.push_back(i);
//cout<<"sampleIndeces"<<sampleIndeces[i]<<endl;
    }
//cout<<"start"<<endl;//////////////m_rootNode   boost::shared_ptr<PairNode>
    m_rootNode->eval(data, sampleIndeces, m_confidences, m_predictions);///??
//cout<<"m_confidences2"<<m_confidences(12,2)<<endl;
//cout<<"end"<<endl;
    //cout<<m_confidences(2,0)<<endl
//<<m_confidences(2,1)<<endl
//<<m_confidences(2,2)<<endl
//<<m_confidences(2,3)<<endl
//<<m_confidences(2,4)<<endl
//<<m_confidences(2,5)<<endl
//<<m_confidences(2,6)<<endl
//<<m_confidences(2,7)<<endl
//<<m_confidences(2,8)<<endl;////////////////////////////////
...
void Tree::eval(const matrix<float>& data, matrix<float>& forestConfidences)
{
    // Initialize
	m_confidences.clear();
	m_predictions.clear();
...
//cout<<"test"<<m_confidences(1, 1)<<endl;///////////////////confidences
    }
}
...
void Tree::subSample(const int numSamples, const std::vector<double>& weights)
{
